...
import keras
from keras.layers import Input, Dense, Dropout, Activation, Concatenate,multiply, BatchNormalization ,Reshape
...
def DenseNet(input_shape=None, dense_blocks=3, dense_layers=-1, growth_rate=12, nb_classes=None, dropout_rate=None,
    ...
    return Model(data_input, x, name=model_name), model_name


def dense_block(x, nb_layers, nb_channels, growth_rate, dropout_rate=None, weight_decay=1e-4):
    """
    ...
    for i in range(nb_layers):
        cb = convolution_block(x, nb_channels, dropout_rate, weight_decay)
        x_list.append(cb)
...
def preprocess(X_train,X_test,y_train,y_test,fac,std=True):
    ...
    X_train = X_train.reshape((X_train.shape[0],X_train.shape[1]//fac,X_train.shape[2]*fac))
    X_test = X_test.reshape((X_test.shape[0],X_test.shape[1]//fac,X_test.shape[2]*fac))
...
def train_eval(X_train,X_test,y_train,y_test,nb_classes,class_weight,batch_size=16,epochs=200):
    ...
    checkpoint = keras.callbacks.ModelCheckpoint("logs/densenet_best_model.h5",
                            monitor='loss',
                            verbose=1,
                            save_best_only=True,
    ...
    #Evaluate
    #model.load_weights("logs/densenet_best_model.h5")
    model.load_weights("weights/netflow_densenet1d.h5")
...
for i in range(10):
    X_tr,X_tst,y_tr,y_tst,y_true,nb_classes,class_weight=preprocess(X_train,X_test,y_train,y_test,fac,std=True)
    score=train_eval(X_tr,X_tst,y_tr,y_tst,nb_classes,class_weight,batch_size=128,epochs=2000)
    cv_scores.append(score*100)
print("%.2f%% (+/- %.2f%%)" % (np.max(cv_scores), np.std(cv_scores)))
